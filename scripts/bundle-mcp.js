/**
 * Bundle pug-mcp into a single self-contained JS file using esbuild.
 * All pug workspace dependencies and the MCP SDK are inlined — only Node.js
...
async function main() {
  ...
  console.log('Bundling pug-mcp...');

  const result = await esbuild.build({
    entryPoints: [path.resolve(__dirname, '..', 'src', 'mcp-server.js')],
    bundle: true,
    platform: 'node',
    target: ['node20'],
    format: 'cjs',
    outfile: path.resolve(__dirname, '..', 'dist', 'pug-mcp-bundled.js'),
  ...
  console.log('Bundle created: dist/pug-mcp-bundled.js');
}
